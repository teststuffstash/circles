...
that wrote the PR (so you don't share the author's blind spots). This file covers the default route —
a **code-fix PR** (the rubric below) and a **dependency/toolchain bump** (the section at the end).
Two more PR kinds have their own rubric files, keyed for task-label routing:

- **build/feature PR** (`task/build`) → `.agents/review-build.md`
- **integration/assembly PR** (a goal branch → master, `task/goal`) → `.agents/review-goal.md`

Until label-based routing exists platform-side, pick the file that matches what the PR actually is,
and say which rubric you applied.
